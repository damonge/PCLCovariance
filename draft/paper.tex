...
\abstract{The disconnected part of the power spectrum covariance matrix (also known as the ``Gaussian'' covariance) is the dominant contribution on large scales for galaxy clustering and weak lensing datasets. The presence of a complicated sky mask causes non-trivial correlations between different Fourier/harmonic modes, which must be accurately characterised in order to obtain reliable cosmological constraints. Unfortunately, an exact calculation of these correlations involve $O(\ell_{\rm max}^6)$ operations that become computationally impractical very quickly. We present an implementation of approximate methods to estimate the Gaussian covariance matrix of spin-0 and spin-2 flat- and curved-sky fields, expanding on existing algorithms. These methods achieve an $O(\ell_{\rm max}^3)$ scaling, which makes the computation of the covariance matrix as fast as the computation of the power spectrum itself. We quantify the accuracy of these method on large-scale structure and weak lensing data, characterised by comparatively complex survey geometries, making use of a large number of Gaussian but otherwise realistic simulations. We show that, using the covariance matrix computing using these approximations we are able to recover the true posterior distribution cosmological parameters to high accuracy. We also quantify the shortcomings of these methods, which become unreliable on the very largest scales, as well as for covariance matrix elements involving lensing $B$ modes. The algorithms presented here are implemented in the public code {\tt NaMaster} \url{https://github.com/LSSTDESC/NaMaster}.}
...
\begin{document}
  ...
  \section{Results}\label{sec:results}
    \subsection{Simulations}\label{ssec:results.sims}
      ...
      \end{equation}
      where $\chi$ is the comoving radial distance, $z\equiv z(\chi)$ is the corresponding redshift in the lightcone, $P(k,z)$ is the matter power spectrum and the window functions are given by \cite{2004PhRvD..70d3009H}\footnote{See \cite{2019ApJS..242....2C} and references therein for details about these theory predictions}:
      \begin{align}
        \nonumber
  ...
  \section{Flat sky}\label{app:flat}
    ...
    \begin{figure}
      ...
      \caption{$\chi^2$ distributions for the autocorrelations of the fields
        of a redshift bin in the flat-sky regime. We compare the distributions
        obtained with the covariance matrix estimated from simulated power
        spectra, and the analytical NKA and Spin-0 methods. In addition, we
        show the theoretical $\chi^2$ distribution, which is almost perfectly
        followed by the others. The reason why the E and B modes are well
        behaved in flat sky is because in this regime ($\ell \in (120,
        17640)$), the shear power spectra is dominated by noise (see
        Fig.~\ref{fig:cl-2bins}). }
