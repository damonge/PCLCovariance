...
\abstract{The disconnected part of the power spectrum covariance matrix (also
  ...
  Fourier/harmonic modes, which must be accurately characterized in order to
  ...
  correlations involves $O(\ell_{\rm max}^6)$ operations that become
  ...
  on existing algorithms developed in the context of CMB analyses.
  These methods achieve an $O(\ell_{\rm max}^3)$
  ...
  using the approximate covariance matrix, we are able to recover the true
  posterior distribution of cosmological parameters to high accuracy. We also
  quantify the shortcomings of these methods, which become unreliable on the
  very largest scales, as well as for covariance matrix elements involving
  cosmic shear $B$ modes. The algorithms presented here are implemented in the
  public code {\tt NaMaster} \url{https://github.com/LSSTDESC/NaMaster}.}
...
\begin{document}
  ...
  \section{Analytical Gaussian covariances}\label{sec:theory}
    \subsection{Preliminaries}\label{ssec:theory.prelim}
      ...
      \end{align}
      where $\summ{k}$ denotes an integral over all possible generalized Fourier coefficients ${\bf k}$, and $\Delta^x$ and $\Delta^k$ are generalized delta functions, defined through their action on functions of ${\bf x}$ or ${\bf k}$:
      \begin{align}
    ...
    \subsection{The pseudo-$C_\ell$ method}\label{ssec:theory.pcl}
      ...
      \begin{enumerate}
        \item We first bin different ${\bf l}$ modes into sets of them called bandpowers (typically bands of similar $\ell$ or annuli of flat-sky Fourier modes spanning a range of radii). Let us denote a given bandpower by its index $q$. We must note that it is more appropriate to use bandpower-averaged spectra when the underlying spectrum does not vary much within each $\ell$ bin. When this is not the case, it is often useful to apply $\ell$-weights (such as $D_\ell\equiv\ell(\ell+1)C_\ell/(2\pi)$). The large-scale structure spectra discussed here are sufficiently flat that the binning used is appropriate. The binned pseudo-power spectrum is:
    ...
    \subsection{Covariance matrices}\label{ssec:theory.pclcov}
      ...
      order to simplify these expressions further, we follow \cite{Efstathiou:2006eb,2017A&A...602A..41C} and neglect all gradients of the masks\footnote{It is worth noting that it is possible in principle to avoid this approximation, as demonstrated in \cite{2005MNRAS.360..509C}.}, which allows us to relate the different $I^{\pm s_a,\pm s_b}$ through the following set of identities:
      \begin{align}\nonumber
  ...
  \section{Results}\label{sec:results}
    ...
    \subsection{Qualitative comparison}\label{ssec:results.visual}
      ...
      \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{./figures/sph_1bin_diff_corr-lims-v02.pdf}
      ...
      \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{./figures/run_sph_2b_NKA_diff_corr_short-lims-v02.pdf}
    ...
    \subsection{Quantitative comparison}\label{ssec:results.quant}
      ...
      covariance matrix elements are needed on these large scales for cosmic shear, they
      can be estimated alternatively making use of fast, low resolution
      simulations (e.g. {\tt HEALPix} $N_{\rm side}=64$ maps), or computed
    ...
    \subsection{Impact on parameter estimation}\label{ssec:results.cosmo}
      ...
      derived from power spectrum measurements. Assuming flat priors and
      a fiducial Gaussian likelihood approximation~\cite{2008PhRvD..77j3013H}, in which the covariance matrix is computed only once for the fiducial model, the posterior distribution for parameters $\vec{\theta}$ is simply given by:
  ...
  \section{Discussion}\label{sec:discussion}
    ...
    experiments, by two factors: 
    the fact that the fields involved are
    non-Gaussian at some level and the arguably higher complexity of the sky masks used in optical datasets.
    ...
    approximations, computationally speaking, the problem of estimating a
    ...
    itself, and scales with the number of pixels in the map as $N_{\rm
      pix}^{3/2}$. We leave for future work the study of the impact of
    ...
    shown that
    %, in spite of the higher complexity of large-scale structure masks,
    ...
    computed making use of fast low-resolution simulations or exactly
  ...
  acknowledges support from the Beecroft Trust.  Some of the results in this
