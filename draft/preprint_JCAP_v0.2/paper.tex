...
\abstract{The disconnected part of the power spectrum covariance matrix (also
  ...
  on existing algorithms \cgg{developed in the context of CMB analyses}.
...
\begin{document}
  ...
  \section{Analytical Gaussian covariances}\label{sec:theory}
    ...
    \subsection{Covariance matrices}\label{ssec:theory.pclcov}
      ...
      order to simplify these expressions further, we follow \cite{Efstathiou:2006eb,2017A&A...602A..41C} and neglect all gradients of the masks\footnote{\cgg{It is worth noting that it is possible in principle to avoid this approximation, as demonstrated in \cite{2005MNRAS.360..509C}}.}, which allows us to relate the different $I^{\pm s_a,\pm s_b}$ through the following set of identities:
      \begin{align}\nonumber
  ...
  \section{Results}\label{sec:results}
    ...
    \subsection{Impact on parameter estimation}\label{ssec:results.cosmo}
      ...
      \end{equation}
      where $\hat{\bf C}$ is a vector of power spectrum measurements, ${\rm Cov}$ is their covariance matrix, and ${\bf C}(\vec{\theta})$ is their theoretical prediction for parameters $\vec{\theta}$. \cgg{It is worth noting that this Gaussian likelihood is not accurate on large scales, where the small number of modes invalidates the application of the central limit theorem. Even in this regime, the likelihood can be approximated through the method described in \cite{2008PhRvD..77j3013H}, which still requires an accurate estimate of the power spectrum covariance.}
  ...
  \section{Discussion}\label{sec:discussion}
    ...
    non-Gaussian at some level and the \cgg{arguably} higher complexity of the sky masks used in optical datasets.
